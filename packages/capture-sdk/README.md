...
**A gap flagged earlier, since closed**: headless Edge's `--screenshot` flag only takes one screenshot after page load with no interactive driving, so an earlier pass through this SDK could confirm the widget reached the passport-capture screen with a fake camera device, but never drove an actual click-through past that — `chromium-cli`/Playwright weren't installed at the time. Playwright was added in a follow-up pass (`test-e2e/live-flow.mjs`, not part of `pnpm test` — it needs a real running backend with real OCR/biometrics sidecars, not stubs, the same way `demo/index.html` is a manual tool, not an automated one) and used to drive the *entire* flow for real: Welcome → all three captures (each waiting for an actual decoded video frame from the fake camera device before clicking Capture — the Capture button can be enabled a moment before frames are actually flowing, and clicking it then is a silent no-op, see `CameraView.tsx`'s own comment on this) → review → confirm, through real upload/OCR/biometrics processing, to the `Submitted` completion screen and the host page's `onComplete` callback actually firing. Zero console errors. The fake camera device's synthetic test pattern (not a real passport/face) correctly produced `NEEDS_REVIEW`/`UNKNOWN` rather than any crash or false positive — the same safe-failure behavior Phase 2's real-photo testing already established for a non-matching input, now confirmed through this SDK's own integration surface too, not just the raw API.

To re-run it yourself: `pnpm exec playwright install chromium` once, serve this package's root (`python -m http.server 8080` or similar) so `demo/index.html`'s relative `../dist/capture-sdk.js` resolves, mint a real session token, then `node test-e2e/live-flow.mjs <sessionToken> <apiBaseUrl> <demoUrl>`.
